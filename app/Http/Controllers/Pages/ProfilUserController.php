...
class ProfilUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = \DB::table('tbl_peminjaman as p')
            ...
            ->select('p.nama_kegiatan', 'p.organisasi', 'p.tanggal_peminjaman', 'p.waktu_peminjaman', 'p.status_persetujuan', 'r.id_ruangan', 'r.nama_ruangan')
            ->where('p.nim', $nim);
        if ($status && in_array($status, ['Disetujui', 'Menunggu', 'Tidak Disetujui'])) {
            $query->where('p.status_persetujuan', $status);
        }
        $result = $query->orderBy('p.tanggal_peminjaman', $sort == 'asc' ? 'asc' : 'desc')->get();
