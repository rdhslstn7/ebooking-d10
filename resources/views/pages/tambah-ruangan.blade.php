...
<body class="bg-light">
  ...
  <div class="container mt-4">
    <h2 class="mb-4"><i class="fas fa-plus me-2"></i>Tambah Ruangan</h2>
    @if($errors->any())
    ...
    <div class="card">
      <div class="card-body">
        <form method="POST" action="{{ url('ruangan-admin') }}" enctype="multipart/form-data">
          @csrf
          <div class="mb-3">
            <label for="id_ruangan" class="form-label">ID Ruangan</label>
            <input type="text" class="form-control" id="id_ruangan" name="id_ruangan" required value="{{ old('id_ruangan') }}">
          </div>
          <div class="mb-3">
            <label for="nama_ruangan" class="form-label">Nama Ruangan</label>
            <input type="text" class="form-control" id="nama_ruangan" name="nama_ruangan" required value="{{ old('nama_ruangan') }}">
          </div>
          <div class="mb-3">
            ...
            <select class="form-select" id="status_ruangan" name="status_ruangan" required>
              <option value="Tersedia" {{ old('status_ruangan') == 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
              <option value="Tidak Tersedia" {{ old('status_ruangan') == 'Tidak Tersedia' ? 'selected' : '' }}>Tidak Tersedia</option>
            </select>
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="number" class="form-control" id="kapasitas" name="kapasitas" min="1" required value="{{ old('kapasitas') }}">
          </div>
          <div class="mb-3">
            <label for="fasilitas" class="form-label">Fasilitas</label>
            <textarea class="form-control" id="fasilitas" name="fasilitas" rows="2" required>{{ old('fasilitas') }}</textarea>
          </div>
